...
\\
\\
By simplifying to a scenario starting at $t=0$, I've calculated that if the gate starts in the on state, then
\[
\Delta t = \begin{cases}
...
\]
And if the gate starts in the off state, then the cases are swapped with some slight adjustment:
\[
\Delta t = \begin{cases}
...
So, perhaps more formally, $w=\max(\Delta t - \Omega,0)$.
