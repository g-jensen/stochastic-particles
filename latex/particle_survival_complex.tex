...
\[
\Delta t = \begin{cases}
  l - \frac{2}{v}, & l \modx (T_1+T_2) \leq T_1\\
  T_1+\lfloor \frac{l}{T_1+T_2}\rfloor(T_1+T_2)-\frac{2}{v}, & l \modx (T_1+T_2) > T_1
\end{cases}
\]
is the latest time that the particle can start at and still live. Since every lap starts at some offset during an interval when the gate is ON, say some $\Omega \modx (T_1+T_2)\leq T_1$. Then we have the following equation regarding the wait time during a lap:
\[
w + \Omega = \Delta t \implies w= \Delta t - \Omega.
\]
And if the gate starts OFF, then the cases are swapped with some slight adjustment:
\[
\Delta t = \begin{cases}
  l - \frac{2}{v}, & l \modx (T_1+T_2) > T_1\\
  \lfloor \frac{l}{T_1+T_2}\rfloor(T_1+T_2)-\frac{2}{v}, & l \modx (T_1+T_2) \leq T_1
\end{cases}
\]
It is possible that $w<0$, implying that the particle will not be able to complete the lap.\\
So, more formally, $w=\max(\Delta t - \Omega,0)$.
