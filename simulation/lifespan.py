...
def load_csv(path):
    ...
    with open(path) as f:
        ...
        next(reader)
        for row in reader:
            phase, on_ratio, _optimal_tt, optimal_ls = [parse_value(c) for c in row]
            results[(phase, on_ratio)] = optimal_ls
    return results
...
def build_grid(results):
    ...
    for (phase, on_ratio), lifespan in results.items():
...
def render(path, phases, on_ratios, grid):
    ...
    lifespans = sorted(set(finite_vals))
    if has_inf:
        lifespans.append(math.inf)

    n_colors = len(lifespans)
    ls_to_index = {ls: i for i, ls in enumerate(lifespans)}
    index_grid = np.full_like(grid, np.nan)
    for yi in range(grid.shape[0]):
        for xi in range(grid.shape[1]):
            val = grid[yi, xi]
            if not np.isnan(val):
                index_grid[yi, xi] = ls_to_index[val]

    all_values = list(phases) + list(on_ratios) + [v for v in lifespans if not math.isinf(v)]
    ...
    cbar.ax.set_yticklabels([fraction_label(lifespans[i], denom_limit) for i in cbar_tick_indices])
    ...
    if show_labels:
        ...
        for yi in range(n_on_ratios):
            for xi in range(n_phases):
                val = grid[yi, xi]
                if not np.isnan(val):
                    label = fraction_label(val, denom_limit)
