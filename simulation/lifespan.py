...
def render(path, phases, on_ratios, grid):
    ...
    all_values = (
        list(phases) + list(on_ratios) + [v for v in lifespans if not math.isinf(v)]
    )
    ...
    if has_inf and (len(lifespans) - 1) not in selected:
        selected.append(len(lifespans) - 1)
    ...
    cbar.set_label(r"$L_{\alpha_{L}^{*}}(\phi, r)$")
